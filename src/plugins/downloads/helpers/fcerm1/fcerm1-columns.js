 ...
 *   scope     {string}   'legacy' | 'new' | 'common' — which format template includes this column
 *   export    {boolean}  If false the column is formula-only; skip writing (default true)
 *   dateRange {boolean}  If true, write 10 values starting at `column` for years 2023-2032 (default false)
...
/** The 10 financial years the spreadsheet covers (fiscal year starting in each) */
...
/** All columns for the new-format FCERM1 template (future) */
export const NEW_COLUMNS = FCERM1_COLUMN_MAP.filter(
  (col) => col.scope === 'new' || col.scope === 'common'
)
